...
$members = Import-CSV "c:\myscripts\servers.csv" -Header servers | select -ExpandProperty Servers
Get-ScheduledTasks -computername $members | select Computername, name, Lastruntime, Nextruntime, status, enabled, NumberofMissedRuns | Export-Csv -Path "c:\myscripts\$(get-date -f yyyy-MM-dd) servertasks.csv" -NoTypeInformation
